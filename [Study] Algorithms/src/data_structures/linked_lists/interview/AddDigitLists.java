...
public class AddDigitLists
{
	 ...
	 * 
	 * - result.getElement() = (node1.getElement() + node2.getElement() + any earlier carry) % 10
	...
	public static Node<Integer> addLists(Node<Integer> list1, Node<Integer> list2, int carry)
	{
		...
		if (list1 != null)
			sum += list1.getElement();
		if (list2 != null)
			sum += list2.getElement();

		// take modulus of the sum to get digit
		int resultDigit = sum % 10;	

		// create the new node in the result list & assign the element
		Node<Integer> result = new Node<Integer>(resultDigit);

		/* the "next" reference will point to the rest of the result */

		int newCarry = sum > 10 ? 1 : 0; // carry is 0 or 1	
		Node<Integer> remainingList1 = (list1 == null ? null: list1.getNext()); // advance the list, or return null
		Node<Integer> remainingList2 = (list2 == null ? null: list2.getNext());  // advance the list, or return null

		// recursive call
		Node<Integer> remainingResult = addLists(remainingList1, remainingList2, newCarry);

		// link the lists
		result.setNext(remainingResult);
